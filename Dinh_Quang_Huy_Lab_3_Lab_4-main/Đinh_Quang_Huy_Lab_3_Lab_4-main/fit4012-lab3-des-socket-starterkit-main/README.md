...
## Ethics & Safe use
- Chỉ chạy hệ thống/demo trên máy cá nhân, máy ảo, hoặc mạng nội bộ được phép (như lab nhà trường).
- Chỉ dùng dữ liệu giả lập để không lộ dữ liệu thật nhạy cảm.
- Không bao giờ sử dụng hệ thống này để quét cổng, dò quét, hay gửi rác tới các máy tính/địa chỉ IP khác không thuộc phạm vi sở hữu.
- Không lấy đoạn mã minh hoạ lab này áp dụng cho bất kỳ hệ thống giao dịch hay sản phẩm thực tế nào (do thiết kế khóa gửi plaintext). Công cụ không sử dụng để thám thính hoặc tấn công.
- Những module ngoài hoặc nếu bạn tham khảo đoạn mã từ Google/StackOverflow, cần ghi chú trích dẫn đầy đủ (Attribution).
